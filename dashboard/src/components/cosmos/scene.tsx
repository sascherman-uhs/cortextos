...
import { PerfToggle, readPerfMode, type PerfMode } from './perf-toggle';
...
// High-perf particle count; low mode uses ~1/4.
const HIGH_PARTICLES = 4000;
const LOW_PARTICLES = 1000;

// === JARVIS MOD #20: state → orb color map ===
// idle gold, listening bright white-ish, processing warm amber, responding gold.
...
export default function Scene() {
  ...
  const orbColor = STATE_COLOR[voiceState];
  // Orb breathing:
  ...
  const particleCount = mode === 'low' ? LOW_PARTICLES : HIGH_PARTICLES;
  // Cap dpr in low mode; allow up to 2 in high mode for crisp wireframe.
  const dpr: [number, number] = mode === 'low' ? [1, 1] : [1, 2];
  ...
  return (
    <div className="relative h-screen w-screen overflow-hidden" style={{ background: CHARCOAL }}>
      <Canvas
      ...
      >
        ...
        {/* === JARVIS MOD #22: point light so orbit spheres catch highlights === */}
        <pointLight position={[0, 0, 6]} intensity={0.6} />
        <Particles count={particleCount} />
        {/* === JARVIS MOD #20: voice state drives orb color + amplitude === */}
        <Orb amplitude={orbAmplitude} color={orbColor} />
        {/* === JARVIS MOD #22: agent orbit spheres (Tier 4) === */}
        <OrbitSystem agents={agents} selectedName={selectedName} onSelect={onSelect} />
        {/* === END JARVIS MOD #22 === */}
      </Canvas>

      {/* JARVIS label under the orb */}
      <div className="pointer-events-none absolute inset-x-0 bottom-[18%] flex justify-center">
        <span className="text-2xl font-light tracking-[0.5em] text-[#CFB383] [text-shadow:0_0_20px_rgba(207,179,131,0.4)]">
          JARVIS
        </span>
      </div>
