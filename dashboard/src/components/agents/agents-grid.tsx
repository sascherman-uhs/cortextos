    ...
    <div className="space-y-4">
      ...
      {agents.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {agents.map((agent) => (
            // Key by systemName (the unique on-disk agent dir) — display `name`
            // comes from IDENTITY.md and collides across un-onboarded template
            // agents (e.g. the hermes/kimi smoke fixtures), causing React
            // duplicate-key errors. systemName is always unique.
            <AgentCard key={agent.systemName} agent={agent} />
          ))}
          <AddAgentCard onClick={() => setCreateOpen(true)} />
        </div>
      )}
