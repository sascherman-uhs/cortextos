...
export async function POST(request: NextRequest) {
  ...
  // === JARVIS MOD #38: any [Cosmos] turn that reaches the inbox path went to
  // the full agent (10-60s think time) — tell the client so it can speak an
  // immediate acknowledgment instead of dead air. ===
  ...
  try {
    ...
    return Response.json(
      // === JARVIS MOD #38: escalated flag (see above) ===
      { success: true, messageId, ...(escalated ? { escalated: true } : {}) },
      { status: 200 },
    );
  } catch (err: unknown) {
    const message = err instanceof Error ? err.message : String(err);
