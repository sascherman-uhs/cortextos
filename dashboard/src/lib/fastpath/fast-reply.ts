// cortextOS Dashboard — Cosmos fast-path reply engine (JARVIS MOD #34, B2/B4/B5)
//
// Direct /v1/messages call for pure-conversational [Cosmos] voice turns.
// ~1–2s instead of the 4–10s (median 10.3s measured 2026-07-06) full
// Claude Code PTY turn. Anything needing tools/data escalates to the real
// agent via the existing inbox (handled by the send route).
//
// Guardrails (plan B5): no tools in the request, conversational only,
// hard timeout so the fast path can never be SLOWER than falling through,
// metrics JSONL with cache_creation/read tokens per call.
//
// Key resolution is REQUEST-TIME (feedback_wire_dormant_integrations_now):
// env ANTHROPIC_API_KEY, else FASTPATH_ANTHROPIC_API_KEY/ANTHROPIC_API_KEY in
// ~/cortextos/orgs/<org>/secrets.env (mtime-cached). No key → 'unavailable'
// and the route falls through to the normal agent path unchanged.

import fs from 'fs';
import path from 'path';
import { getAgentDir, getLogDir, getFrameworkRoot } from '@/lib/config';
import { assembleSystem } from './identity-assembler';
import { buildWindow, type Turn } from './conversation-window';
import { extractSentences } from './stream-sentences';
// === JARVIS MOD #66 — authoritative live counts injected per turn ===
import { getSnapshotBlock, getLiveCounts } from './live-snapshot';
// === JARVIS MOD #68 — observe (never gate) unsourced figures in replies ===
import { auditFigures, numbersIn, summarizeUnsourced } from './figure-audit';

const API_URL = 'https://api.anthropic.com/v1/messages';
const DEFAULT_MODEL = process.env.FASTPATH_MODEL || 'claude-haiku-4-5-20251001';
const TIMEOUT_MS = parseInt(process.env.FASTPATH_TIMEOUT_MS || '6000', 10);
const MAX_TOKENS = 500;
const ESCALATE_TOKEN = '<<ESCALATE>>';

// === JARVIS MOD #69 — ONE escalation predicate, used for BOTH routing and
// logging. Previously the two drifted apart (see the metrics fix below). ===
function isEscalation(text: string): boolean {
  return !text || text.includes(ESCALATE_TOKEN);
}

export type FastReplyResult =
  | { kind: 'reply'; text: string; latencyMs: number }
  | { kind: 'escalate' }
  | { kind: 'unavailable' } // no API key configured — use normal path
  | { kind: 'error'; error: string };

// --- request-time key lookup, mtime-cached ---------------------------------

let keyCache: { key: string | null; mtimeMs: number; path: string } | null = null;

function resolveApiKey(org: string): string | null {
  if (process.env.ANTHROPIC_API_KEY) return process.env.ANTHROPIC_API_KEY;
  const secretsPath = path.join(getFrameworkRoot(), 'orgs', org, 'secrets.env');
  let mtimeMs = -1;
  try { mtimeMs = fs.statSync(secretsPath).mtimeMs; } catch { return null; }
  if (keyCache && keyCache.path === secretsPath && keyCache.mtimeMs === mtimeMs) {
    return keyCache.key;
  }
  let key: string | null = null;
  try {
    for (const line of fs.readFileSync(secretsPath, 'utf-8').split('\n')) {
      const m = line.match(/^(?:export\s+)?(?:FASTPATH_)?ANTHROPIC_API_KEY=["']?([^"'\s]+)/);
      if (m) { key = m[1]; break; }
    }
  } catch { /* unreadable → null */ }
  keyCache = { key, mtimeMs, path: secretsPath };
  return key;
}

// --- voice cue (B2): payload-only, never persisted --------------------------

let cueCache: { text: string; mtimeMs: number; agent: string } | null = null;

export function readVoiceCue(agent: string, org: string): string {
  const cuePath = path.join(getAgentDir(agent, org), 'VOICE_CUE.md');
  let mtimeMs = -1;
  try { mtimeMs = fs.statSync(cuePath).mtimeMs; } catch { return ''; }
  if (cueCache && cueCache.agent === agent && cueCache.mtimeMs === mtimeMs) {
    return cueCache.text;
  }
  let text = '';
  try { text = fs.readFileSync(cuePath, 'utf-8').trim(); } catch { /* '' */ }
  cueCache = { text, mtimeMs, agent };
  return text;
}

/**
 * Append the voice cue to the LAST user message in the payload only.
 * The stored logs never contain the cue (unit-tested invariant).
 */
export function applyVoiceCue(messages: Turn[], cue: string): Array<{ role: string; content: string }> {
  const out = messages.map((m) => ({ role: m.role, content: m.content }));
  if (cue) {
    for (let i = out.length - 1; i >= 0; i--) {
      if (out[i].role === 'user') {
        out[i] = { ...out[i], content: `${out[i].content}\n\n<voice_cue>\n${cue}\n</voice_cue>` };
        break;
      }
    }
  }
  return out;
}

// --- metrics (B5) ------------------------------------------------------------

function logMetrics(agent: string, entry: Record<string, unknown>): void {
  try {
    const logDir = getLogDir(agent);
    fs.mkdirSync(logDir, { recursive: true });
    fs.appendFileSync(
      path.join(logDir, 'fastpath-metrics.jsonl'),
      JSON.stringify({ timestamp: new Date().toISOString(), ...entry }) + '\n',
    );
  } catch { /* metrics must never break the reply path */ }
}

// === JARVIS MOD #68 — fabrication telemetry ==================================
// Logs figures a reply asserts that nothing authorized. OBSERVER ONLY: it never
// edits or suppresses a reply, because a regex cannot reliably tell a grounded
// number from a guessed one, and silently eating a correct sentence is worse
// than a noisy log line. Its value is making fabrication visible in
// fastpath-metrics.jsonl instead of needing a hand-run 25-turn soak to find it.
async function auditReply(replyText: string, userText: string): Promise<string[] | null> {
  try {
    const counts = await getLiveCounts();
    const allowedNumbers = counts
      ? [counts.activeStagings, counts.openContracts, counts.awaitingInstall]
      : [];
    // Scott's own words ground anything he supplied himself this turn.
    const result = auditFigures(replyText, allowedNumbers, userText);
    return result.unsourced.length ? summarizeUnsourced(result) : null;
  } catch {
    return null; // telemetry must never break the reply path
  }
}
// === END MOD #68 =============================================================

// --- main entry (B4) ---------------------------------------------------------

// === JARVIS MOD #45 (Phase 3): shared request prep for both entrypoints ===
function prepareMessages(agent: string, org: string, userText: string, started: number, snapshot = '') {
  const system = assembleSystem(agent, org, undefined, snapshot);
  let window = buildWindow(agent);
  // Ensure the current turn is the last user message even if the log append
  // hasn't landed yet (defensive — caller appends before calling us).
  const last = window[window.length - 1];
  if (!last || last.role !== 'user' || !last.content.includes(userText)) {
    window = [...window, { role: 'user', content: userText, ts: started }];
  }
  // === JARVIS MOD #67c — break the laundering chain ==========================
  // Re-running the soak showed the lane echoing its OWN earlier inventions
  // ("unsigned since Friday", "crews start at five") with total confidence: a
  // figure it guessed three turns ago sits in the window looking exactly like
  // a figure it retrieved, and prompt text alone loses to that precedent.
  //
  // So the payload marks them. Every prior assistant turn is re-audited and any
  // that asserted figures nothing grounded gets an inline [unverified] tag, so
  // the model can SEE which of its past numbers were guesses instead of having
  // to take its own word for them.
  //
  // Payload-only, exactly like the voice cue: the stored logs are untouched, so
  // history stays byte-identical and nothing user-visible is rewritten.
  const allowed = numbersIn(snapshot);
  const marked = window.map((turn) => {
    if (turn.role !== 'assistant') return turn;
    if (auditFigures(turn.content, allowed).unsourced.length === 0) return turn;
    return {
      ...turn,
      content:
        `${turn.content}\n[unverified: figures in this earlier reply came from no data ` +
        `source — do NOT repeat them as fact]`,
    };
  });
  const messages = applyVoiceCue(marked, readVoiceCue(agent, org));
  return { system, messages };
}
// === END MOD #45 ===

/**
 * Attempt a fast conversational reply for a [Cosmos] user turn.
 * `userText` is the message WITHOUT the [Cosmos] prefix.
 * The caller has already appended the turn to inbound-messages.jsonl, so the
 * window read here includes it.
 */
export async function tryFastReply(
  agent: string,
  org: string,
  userText: string,
): Promise<FastReplyResult> {
  const apiKey = resolveApiKey(org);
  if (!apiKey) return { kind: 'unavailable' };

  const started = Date.now();
  try {
    // MOD #66: fetch before building the prompt. 60s-cached, so this is a
    // no-op on nearly every turn; a failure yields '' and the NUMBERS RULE
    // routes count questions to the full agent instead of to a guess.
    const snapshot = await getSnapshotBlock();
    const { system, messages } = prepareMessages(agent, org, userText, started, snapshot);

    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), TIMEOUT_MS);
    let resp: Response;
    try {
      resp = await fetch(API_URL, {
        method: 'POST',
        signal: controller.signal,
        headers: {
          'content-type': 'application/json',
          'x-api-key': apiKey,
          'anthropic-version': '2023-06-01',
        },
        body: JSON.stringify({
          model: DEFAULT_MODEL,
          max_tokens: MAX_TOKENS,
          system,
          messages,
        }),
      });
    } finally {
      clearTimeout(timer);
    }

    const latencyMs = Date.now() - started;
    if (!resp.ok) {
      const errBody = await resp.text().catch(() => '');
      logMetrics(agent, { event: 'error', status: resp.status, latencyMs, error: errBody.slice(0, 300) });
      return { kind: 'error', error: `HTTP ${resp.status}` };
    }

    const data = await resp.json() as {
      content?: Array<{ type: string; text?: string }>;
      usage?: {
        input_tokens?: number; output_tokens?: number;
        cache_creation_input_tokens?: number; cache_read_input_tokens?: number;
      };
    };
    const text = (data.content ?? [])
      .filter((b) => b.type === 'text' && b.text)
      .map((b) => b.text as string)
      .join('')
      .trim();

    const u = data.usage ?? {};
    const read = u.cache_read_input_tokens ?? 0;
    const write = u.cache_creation_input_tokens ?? 0;
    const denom = read + write + (u.input_tokens ?? 0);
    const unsourced = isEscalation(text) ? null : await auditReply(text, userText);
    logMetrics(agent, {
      // === JARVIS MOD #69 — label from the SAME predicate the router uses.
      // This logged exact-match while routing on .includes(), so turns that
      // wrapped the token in prose were routed correctly and logged as
      // 'reply' — metrics claimed 14 replies where 11 happened, inflating the
      // fast-path hit rate. A metric that disagrees with the behaviour it
      // measures is worse than no metric. (Found by the 2026-08-03 soak.)
      event: isEscalation(text) ? 'escalate' : 'reply',
      model: DEFAULT_MODEL,
      latencyMs,
      ...(unsourced ? { unsourced_figures: unsourced } : {}),
      input_tokens: u.input_tokens ?? 0,
      output_tokens: u.output_tokens ?? 0,
      cache_creation_input_tokens: write,
      cache_read_input_tokens: read,
      cache_read_ratio: denom ? Math.round((read / denom) * 100) / 100 : 0,
    });

    if (isEscalation(text)) {
      return { kind: 'escalate' };
    }
    return { kind: 'reply', text, latencyMs };
  } catch (err: unknown) {
    const latencyMs = Date.now() - started;
    const msg = err instanceof Error ? (err.name === 'AbortError' ? 'timeout' : err.message) : String(err);
    logMetrics(agent, { event: 'error', latencyMs, error: msg.slice(0, 300) });
    return { kind: 'error', error: msg };
  }
}

// === JARVIS MOD #45 (Phase 3): streaming fast reply ==========================
// Same call as tryFastReply but with `stream: true` — complete sentences are
// handed to `onSentence` the moment their boundary lands, so the client can
// start TTS while the rest of the reply is still generating. The returned
// result carries the FULL text (the caller appends one full-text line to
// outbound-messages.jsonl, so history is byte-identical to the non-stream
// path). Escalation safety: extractSentences holds anything containing '<',
// so no fragment of an `<<ESCALATE>>` turn is ever flushed aloud.

export async function tryFastReplyStream(
  agent: string,
  org: string,
  userText: string,
  onSentence: (sentence: string) => void,
): Promise<FastReplyResult> {
  const apiKey = resolveApiKey(org);
  if (!apiKey) return { kind: 'unavailable' };

  const started = Date.now();
  try {
    // MOD #66: fetch before building the prompt. 60s-cached, so this is a
    // no-op on nearly every turn; a failure yields '' and the NUMBERS RULE
    // routes count questions to the full agent instead of to a guess.
    const snapshot = await getSnapshotBlock();
    const { system, messages } = prepareMessages(agent, org, userText, started, snapshot);

    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), TIMEOUT_MS);
    let full = '';
    let pending = '';
    let flushed = 0;
    let ttfsMs: number | null = null;
    const usage: Record<string, number> = {};
    try {
      const resp = await fetch(API_URL, {
        method: 'POST',
        signal: controller.signal,
        headers: {
          'content-type': 'application/json',
          'x-api-key': apiKey,
          'anthropic-version': '2023-06-01',
        },
        body: JSON.stringify({
          model: DEFAULT_MODEL,
          max_tokens: MAX_TOKENS,
          system,
          messages,
          stream: true,
        }),
      });

      if (!resp.ok || !resp.body) {
        const errBody = await resp.text().catch(() => '');
        logMetrics(agent, {
          event: 'error', status: resp.status, latencyMs: Date.now() - started,
          error: errBody.slice(0, 300), stream: true,
        });
        return { kind: 'error', error: `HTTP ${resp.status}` };
      }

      // Anthropic SSE: lines of `event: X` / `data: {...}` separated by blank lines.
      const reader = resp.body.getReader();
      const decoder = new TextDecoder();
      let sseBuf = '';
      for (;;) {
        const { done, value } = await reader.read();
        if (done) break;
        sseBuf += decoder.decode(value, { stream: true });
        let nl: number;
        while ((nl = sseBuf.indexOf('\n')) !== -1) {
          const line = sseBuf.slice(0, nl).trim();
          sseBuf = sseBuf.slice(nl + 1);
          if (!line.startsWith('data:')) continue;
          let evt: {
            type?: string;
            delta?: { type?: string; text?: string };
            message?: { usage?: Record<string, number> };
            usage?: Record<string, number>;
          };
          try { evt = JSON.parse(line.slice(5).trim()); } catch { continue; }
          if (evt.type === 'content_block_delta' && evt.delta?.type === 'text_delta' && evt.delta.text) {
            full += evt.delta.text;
            pending += evt.delta.text;
            const { sentences, rest } = extractSentences(pending);
            pending = rest;
            for (const s of sentences) {
              if (ttfsMs === null) ttfsMs = Date.now() - started;
              flushed += 1;
              onSentence(s);
            }
          } else if (evt.type === 'message_start' && evt.message?.usage) {
            Object.assign(usage, evt.message.usage);
          } else if (evt.type === 'message_delta' && evt.usage) {
            Object.assign(usage, evt.usage);
          }
        }
      }
    } finally {
      clearTimeout(timer);
    }

    const latencyMs = Date.now() - started;
    const text = full.trim();
    // Flush the unfinished tail (a final sentence rarely gets trailing
    // whitespace, so it never crosses extractSentences's boundary). Only when
    // sentences already streamed — the client speaks EITHER the stream events
    // OR the final replyText, so a partial stream must carry every word.
    if (flushed > 0 && pending.trim() && !text.includes(ESCALATE_TOKEN)) {
      flushed += 1;
      onSentence(pending.trim());
    }
    const unsourcedStream = isEscalation(text) ? null : await auditReply(text, userText);
    const read = usage.cache_read_input_tokens ?? 0;
    const write = usage.cache_creation_input_tokens ?? 0;
    const denom = read + write + (usage.input_tokens ?? 0);
    logMetrics(agent, {
      event: isEscalation(text) ? 'escalate' : 'reply',
      model: DEFAULT_MODEL,
      latencyMs,
      stream: true,
      ...(unsourcedStream ? { unsourced_figures: unsourcedStream } : {}),
      ttfsMs,
      sentences_flushed: flushed,
      input_tokens: usage.input_tokens ?? 0,
      output_tokens: usage.output_tokens ?? 0,
      cache_creation_input_tokens: write,
      cache_read_input_tokens: read,
      cache_read_ratio: denom ? Math.round((read / denom) * 100) / 100 : 0,
    });

    if (isEscalation(text)) {
      return { kind: 'escalate' };
    }
    return { kind: 'reply', text, latencyMs };
  } catch (err: unknown) {
    const latencyMs = Date.now() - started;
    const msg = err instanceof Error ? (err.name === 'AbortError' ? 'timeout' : err.message) : String(err);
    logMetrics(agent, { event: 'error', latencyMs, error: msg.slice(0, 300), stream: true });
    return { kind: 'error', error: msg };
  }
}
// === END MOD #45 =============================================================
