...
export const { handlers, auth, signIn, signOut } = NextAuth({
  ...
  //
  // The Secure flag must follow the actual serving protocol, NOT NODE_ENV.
  // `next start` forces NODE_ENV=production even when the dashboard is served
  // over plain http://localhost; gating Secure on NODE_ENV then marked the
  // csrf cookie Secure, so browsers dropped it on the http POST and Auth.js
  // rejected the sign-in with MissingCSRF. Gate on AUTH_URL's scheme instead.
  cookies: {
    sessionToken: {
      name: 'authjs.session-token',
      options: { httpOnly: true, sameSite: 'lax', path: '/', secure: (process.env.AUTH_URL ?? '').startsWith('https') },
    },
    csrfToken: {
    ...
    callbackUrl: {
      ...
      options: { sameSite: 'lax', path: '/', secure: (process.env.AUTH_URL ?? '').startsWith('https') },
    },
    pkceCodeVerifier: {
