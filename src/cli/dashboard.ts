...
export const dashboardCommand = new Command('dashboard')
  ...
  .action(async (options: { port: string; instance: string; build?: boolean; install?: boolean }) => {
    ...
    // SUPABASE_URL / SUPABASE_KEY: env > dashboard.env — must be persisted so the
    // supa_ task fast-path (dashboard/src/app/api/tasks/[id]/route.ts) can reach the
    // JARVIS Supabase project. MOD #9 (2026-06-07): without this, every dashboard
    // restart regenerates .env.local from dashCreds and DROPS these manually-added
    // keys, breaking supa_ task reads/PATCH/DELETE. Mirrors the MOD #4 AUTH_URL pattern.
